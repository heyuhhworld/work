const reportData = {
    report1: {
        ...
        pmComment: {
            user: '产品经理',
            content: '客户表示对我们的产品非常感兴趣，并有明确的采购意向。建议尽快跟进报价和商务谈判。',
            date: '2024-03-16'
        }
    },
    report2: {
...
function openReport(reportId) {
    ...
    editButtons.forEach(button => {
        button.style.display = reportId === 'report2' ? 'block' : 'none';
    });
    ...
    // 如果是报告1，显示产品经理评论
    if (reportId === 'report1' && report.pmComment) {
        const commentsSection = document.querySelector('.comments-section');
        const pmCommentHtml = `
            <div class="pm-comment-section">
                <div class="pm-tag">产品经理评论</div>
                <p class="date">${report.pmComment.date}</p>
                <p class="content">
                    <strong>${report.pmComment.user}：</strong>
                    ${report.pmComment.content}
                </p>
            </div>
        `;
        // 在评论区域的开头插入产品经理评论
        commentsSection.insertAdjacentHTML('afterbegin', pmCommentHtml);
    }
...
function renderTimelineItems() {
    ...
    timelineContainer.innerHTML = timelineEvents.map(event => {
        switch(event.type) {
            case 'report':
                const report = reportData[event.id];
                return `
                    <div class="timeline-item">
                        ...
                        <div class="timeline-content">
                            <div class="report-card" onclick="openReport('${event.id}')" data-report-id="${event.id}">
                                ...
                                ${event.id === 'report1' && report.pmComment ? `
                                    <div class="pm-comment-section">
                                        <div class="pm-tag">产品经理评论</div>
                                        <p class="date">${report.pmComment.date}</p>
                                        <p class="content">
